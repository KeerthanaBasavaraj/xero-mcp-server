...
export default CreateXeroTool(
  ...
  "Generates a comprehensive business insight report with rate-limited API calls. Includes financial metrics, client analysis, revenue & expenses breakdown, and profit & loss data with variances.",
  {
  ...
  async ({ month }: { month: string }) => {
    ...
    const apiCalls = [
      ...
      () => listXeroProfitAndLoss(prevStartDate, prevEndDateStr, 1, "MONTH", true, false), // Previous month P&L
