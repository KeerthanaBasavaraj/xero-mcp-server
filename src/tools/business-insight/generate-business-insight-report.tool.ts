...
export default CreateXeroTool(
  ...
  "Generates a business insight report with rate-limited API calls. Can return either raw data or a comprehensive processed report with calculated metrics and insights.",
  {
    month: z.string().describe("Month in YYYY-MM format"),
    format: z.enum(["raw", "comprehensive"]).optional().describe("Report format: 'raw' for JSON data, 'comprehensive' for processed report with insights (default: comprehensive)"),
  },
  async ({ month, format = "comprehensive" }: { month: string; format?: "raw" | "comprehensive" }) => {
    ...
    if (format === "raw") {
      const reportData = {
        ...
        errors: {
          ...
          profitAndLossPrev: profitAndLossPrev.isError ? profitAndLossPrev.error : null,
          ...
          organisationDetails: organisationDetails.isError ? organisationDetails.error : null,
    ...
    // Calculate financial metrics for comprehensive report
